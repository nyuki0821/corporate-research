...
- **処理状況確認**: 現在のプロセス状況を表示
- **自動継続設定**: 自動継続機能の有効/無効を切り替え
- **自動継続状況確認**: 自動継続機能の詳細状況を表示
...
### 概要
企業リストに未処理または処理中の企業がある限り、バッチ処理を自動的に継続する機能です。Google Apps Scriptの6分実行制限を考慮した設計になっています。

### 動作仕様
- **6分タイムアウト**: Google Apps Scriptの制限により、1回の実行は最大6分
- **自動再開**: タイムアウト後、5秒待機してから自動的に次のバッチを開始
- **継続条件**: 企業リストに「未処理」または「処理中」ステータスの企業が存在する限り継続
- **自動停止**: 全ての企業の処理が完了すると自動的に停止
...
2. **設定値**:
   - `ENABLE_AUTO_CONTINUE`: 'true'で有効、'false'で無効
   - `AUTO_CONTINUE_DELAY_MS`: 自動継続時の待機時間（デフォルト5秒）

### 状況確認
- **メニューから**: 「バッチ処理」→「自動継続状況確認」
- **表示内容**:
  - 設定状態（有効/無効）
  - 未処理企業数
  - 推定処理時間
...
### 手動制御
- **停止**: 「バッチ処理停止」で自動継続も含めて完全停止
- **再開**: 停止後は「バッチ処理開始」で手動再開が必要

### 注意事項
- 自動継続中はGoogle Apps Scriptのトリガーを使用
- 大量の企業を処理する場合は数時間継続する可能性があります
- エラーが発生しても軽微な場合は継続、重大なエラーで停止
...
### バッチ処理設定
```javascript
BATCH_SIZE: '8'                 // 同時処理企業数（6分制限対応）
MAX_RETRY_COUNT: '3'            // リトライ回数
PROCESSING_DELAY_MS: '2000'     // 処理間隔
ENABLE_AUTO_CONTINUE: 'true'    // 自動継続機能
AUTO_CONTINUE_DELAY_MS: '5000'  // 自動継続待機時間
...
#### Tavily API
- **Basic Search**: $0.008/検索（2クレジット × $0.004/クレジット）
- **Advanced Search**: $0.016/検索（4クレジット × $0.004/クレジット）
- **本システム使用**: Advanced Search（高精度検索のため）

#### OpenAI API
- **GPT-4o mini**: 入力 $0.15/100万トークン、出力 $0.6/100万トークン
- **本システム使用**: GPT-4o mini（コスト効率重視）

### 100件あたりの費用計算

#### 1社あたりのAPI使用量
- **Tavily検索**: 4回/社（公式サイト、基本情報、ニュース、採用情報）
- **OpenAI処理**: 約15,000トークン/社（入力12,000 + 出力3,000）

#### 1社あたりの費用
```
Tavily API: 4検索 × $0.008 = $0.032
OpenAI API: 
  - 入力: 33,000トークン × $0.15/100万 = $0.00495
  - 出力: 500トークン × $0.6/100万 = $0.0003
  - 小計: $0.00525
合計: $0.03725/社（約5.6円/社）
```

#### 100件あたりの費用
- **Tavily API**: $3.20（約480円）
- **OpenAI API**: $0.525（約79円）
- **合計**: **$3.725（約559円）**
...
### 費用削減のポイント
1. **GPT-4o mini使用**: 従来のGPT-4比で90%削減
2. **公式サイト優先検索**: 不要な検索を削減
3. **効率的なバッチ処理**: API呼び出し最適化
4. **自動継続機能**: 手動作業コスト削減

*為替レート: 1USD = 150円で計算（2025年1月時点）
